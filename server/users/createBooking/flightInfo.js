...
export const flightInfo = (req, res) => {
  ...
  if (req.body.isReturn === 0 || req.body.isReturn === "0") {
    db.query(sql, [req.body.departureFlightID], (err, result) => {
      if (err)
        res.json({
          Error: "Error while selecting flight info.",
        });
      // Todo - send Error message
      else return res.json({ Status: "Success", DepFlight: result[0] });
    });
    return;
  } else if (req.body.isReturn === 1 || req.body.isReturn === "1") {
    db.query(sql, [req.body.departureFlightID], (err, result) => {
      if (err)
        res.json({ Error: "Error while selecting departure flight info." }); // Todo - send Error message
      if (result.length > 0) {
        const dep = result[0];
        db.query(sql, [req.body.returnFlightID], (err, result) => {
          if (err)
            res.json({ Error: "Error while selecting return flight info." }); // Todo - send Error message
          if (result.length > 0) {
            const ret = result[0];
            return res.json({
              Status: "Success",
            ...
            return res.json({
              Error: "No return flight found.",
              DepFlight: dep,
            });
        });
      } else return res.json({ Error: "No departure flight found." });
    });
  }
};
