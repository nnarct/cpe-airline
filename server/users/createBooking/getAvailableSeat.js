...
export const getAvailableSeat = (req, res) => {
  ...
  const sqlAllSeat =
    "SELECT s.*, c.Name AS Class FROM seat s INNER JOIN class c ON c.ClassID = s.ClassID INNER JOIN plane pl ON pl.PlaneID = c.PlaneID INNER JOIN flight f ON f.PlaneID = pl.PlaneID AND f.FlightID = ? WHERE pl.AirlineID = (SELECT AirlineID FROM flight WHERE FlightID = ?) ORDER BY s.SeatCode";
