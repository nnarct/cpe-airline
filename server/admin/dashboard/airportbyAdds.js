...
export const getAddonsCountByAirport = (req, res) => {
  // const query = `SELECT a.AirportID, a.IATA, COUNT(ad.AddOnsID) AS AddonsCount FROM airport AS a JOIN flight AS f ON a.AirportID = f.DestinationAirportID JOIN addons AS ad ON f.FlightID = ad.FlightID GROUP BY a.AirportID, a.IATA;
  // `;
  const query = `  SELECT a.AirportID, a.IATA, SUM(CASE WHEN AddOnsID IS NULL THEN 0 ELSE 1 END) AS AddonsCount
  FROM airport AS a
